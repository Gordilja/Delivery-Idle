...
namespace QPathFinder
{
    ...
    public class PathFinderEditor : Editor 
    {
        ...
        {
            if (GameObject.FindFirstObjectByType<PathFinder>() == null)
            {
    ...
    #region On Scene Rendering and Scene GUI
        ...
        private void DrawNodes( Color color, bool canMove = false, Node selectedNode = null, Color colorForSelected = default(Color) )
        {
            ...
            foreach (var node in script.graphData.nodes)
            {
                ...
                if ( canMove ) 
                    { var fmh_250_78_638670384925814883 = Quaternion.identity; node.SetPosition ( Handles.FreeMoveHandle(node.Position, script.graphData.nodeSize, Vector3.zero, Handles.SphereHandleCap) ); }
                else
